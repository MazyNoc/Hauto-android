...
public class SwitchViewHolder extends BaseViewHolder<SwitchUnit, SwitchRowBinding> implements View.OnClickListener {
    ...
    public void toggle() {
        ...
        //Server.getInstance().getService().setOutlet(data.id, unit.isOn() ? "start" : "stop")

        Server.getInstance().getService().execute(
            new UnitCommandList(new UnitCommand(data.id, data.createCommand(unit.isOn() ? "on" : "off")))
